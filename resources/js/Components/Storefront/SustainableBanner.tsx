import { Award, CheckCircle2, Clock, DollarSign, Headset, MessageSquare, ShieldCheck, Sparkles, Wrench } from 'lucide-react';
...
export default function SustainableBanner() {
	return (
		<section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 mt-16 sm:mt-24">
			<div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0B0F17] via-[#0F172A] to-[#1E293B] text-white p-8 sm:p-12 shadow-xl border border-slate-800">
				<div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-cyan-500/10" />
				<div className="absolute -left-20 -bottom-32 h-72 w-72 rounded-full bg-blue-500/5" />

				<div className="relative mb-10 text-center max-w-2xl mx-auto">
					<span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3.5 py-1 text-[11px] font-bold tracking-wider uppercase text-[#38BDF8] ring-1 ring-white/10">
				...
				<div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					{advantages.slice(0, 4).map(({ icon: Icon, title, body }) => (
						<div
							key={title}
							className="rounded-2xl bg-white/5 backdrop-blur-sm p-5 border border-white/10 hover:border-[#0284C7] transition-all hover:bg-white/10 group"
						>
							<div className="h-10 w-10 rounded-xl bg-[#0284C7] text-white flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
								<Icon className="h-5 w-5" />
							</div>
							<h3 className="text-base font-bold text-white mt-4">{title}</h3>
							<p className="text-xs text-slate-300 mt-1.5 leading-relaxed font-normal">{body}</p>
						</div>
					))}
				</div>

				<div className="relative grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
					{advantages.slice(4, 7).map(({ icon: Icon, title, body }) => (
						<div
							key={title}
							className="rounded-2xl bg-white/5 backdrop-blur-sm p-5 border border-white/10 hover:border-[#0284C7] transition-all hover:bg-white/10 group"
						>
